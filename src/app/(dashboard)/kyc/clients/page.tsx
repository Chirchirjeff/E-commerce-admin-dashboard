...
import {
  UserCheck, UserX, Clock, Eye, CheckCircle2, XCircle, Loader2,
  RefreshCw, AlertCircle, X, FileText, ExternalLink, User, Phone,
  Mail, MapPin, Hash, Calendar, ChevronDown, ChevronUp, ShieldCheck,
  Building2,
} from 'lucide-react';
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex flex-col w-full max-w-2xl bg-background shadow-2xl overflow-hidden animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <section className="space-y-3">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Registration Details</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <InfoRow icon={Building2} label="Business / entity name" value={record.businessName} />
              <InfoRow icon={Hash}      label="Tax ID / Registration #" value={record.taxId} />
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <DocViewer label="Supporting Document" path={record.businessLicense} />
            </div>
          </section>
              ...
              {showRejectForm && (
                <div className="space-y-1.5 rounded-xl border border-destructive/30 bg-destructive/5 p-4">
                  ...
                  <textarea
                    ...
                    placeholder="Explain why this submission is being rejected so the client can resubmit correctly…"
                    value={rejectReason}
        ...
        {isPending && (
          <div className="flex-shrink-0 border-t bg-background px-6 py-4 space-y-3">
              ...
              <div className="flex gap-3">
                ...
                >
                  {isApproving
                    ...
                    : <><UserCheck className="mr-2 h-4 w-4" />Verify Client</>
                  }
                </Button>
            ...
            <p className="text-xs text-center text-muted-foreground">
              {showRejectForm
                ? 'The client will be notified and can resubmit with the corrected documents.'
                : 'Verifying will grant this client full platform access.'}
            </p>
          </div>
        )}
...
function KYCClientsContent() {
  ...
  const { data: records = [], isLoading, isError, refetch } = useQuery<KYCRecord[]>({
    queryKey: ['kyc', 'clients', 'all', statusFilter],
    queryFn: async () => {
  ...
  // Detail query — fetches full record with docs + review history
  const { data: detail } = useQuery<KYCRecord>({
    queryKey: ['kyc', 'clients', 'detail', selectedId],
    queryFn: async () => {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Client verified', 'The client has been verified and now has full platform access.');
      queryClient.invalidateQueries({ queryKey: ['kyc', 'clients'] });
      setSelectedId(null);
    },
    onError: () => toast.error('Verification failed', 'Something went wrong. Please try again.'),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Client rejected', 'The client has been notified and can resubmit.');
      queryClient.invalidateQueries({ queryKey: ['kyc', 'clients'] });
      setSelectedId(null);
    },
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Client Verification</h1>
            <p className="text-sm text-muted-foreground">Review and verify client identity submissions</p>
          </div>
              ...
              <CardHeader className="pb-2">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="space-y-0.5 min-w-0">
                    <CardTitle className="text-base truncate">{record.user.name}</CardTitle>
                    <CardDescription className="truncate">{record.user.email}</CardDescription>
                  </div>
                  <StatusBadge status={record.status} />
                </div>
              </CardHeader>
